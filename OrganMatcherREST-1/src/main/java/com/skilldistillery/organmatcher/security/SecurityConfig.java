...
public class SecurityConfig extends WebSecurityConfigurerAdapter {
    ...
    protected void configure(HttpSecurity http) throws Exception {
        ...
        .antMatchers(HttpMethod.GET, "/api/patients").permitAll() // For CORS, the preflight request
//        .antMatchers("/api/**").authenticated() // Requests for our REST API must be authorized.
    ...
    protected void configure(AuthenticationManagerBuilder auth) throws Exception {
        String userQuery = "SELECT username, password, enabled FROM auth_user WHERE username=?";
        String authQuery = "SELECT username, role FROM auth_user WHERE username=?";
        auth
        .jdbcAuthentication()
